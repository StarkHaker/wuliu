...
<html lang="zh-CN">
  ...
  <body>
    ...
    <div class="panel panel-default">
      <div class="panel-heading">
		  ...
		  <div class="form-group">
		    <label for="exampleInputName2">菜单管理&nbsp;&nbsp;&nbsp;角色名</label>
		    <input type="text" class="form-control" value="${Mname}" id="menuname" placeholder="Jane Doe">
		  </div>
      ...
      <div class="panel-body">
        <table class="table table-bordered table-hover">
          ...
          <c:forEach items="${list}" var="menu" varStatus="start">
          	  <tr>
          	  	<td><input type="checkbox" value="${menu.mid}" /></td>
	            <td>${start.index + 1}</td>
	            <td>${menu.mname}</td>
	            <td>${menu.pid}</td>
	            <td>${menu.murl}</td>
    ...
    <form action="${basePath}menu/update" id="updateForm" method="post">
    	<input type="hidden" id="_Mid" name="Mid" value=""/>
    </form>
    
    <form action="${basePath}menu/serch" id="pageForm" method="post">
	...
	<script type="text/javascript">
		function page(pagenum){
	   	...
	   	$("#find").click(function(){
	   		var url = "${basePath}menu/"+$("#menuname").val();
	   		$("#findForm").attr("action",url);
	   		$("#findForm").submit();
	   	});
	   	
	   	$("#add").click(function(){
	   		window.location.href="${basePath}menu/add";
	   	});
	   	
	   	//$('input[type=checkbox]:checked')
	   	$("#delete").click(function(){
	   		if(check()){
	   			var url = "${basePath}menu/"+$('input[type=checkbox]:checked').val();
