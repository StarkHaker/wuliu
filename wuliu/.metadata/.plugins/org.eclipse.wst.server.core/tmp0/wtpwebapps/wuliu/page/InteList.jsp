...
<html lang="zh-CN">
  ...
  <body>
    ...
    <div class="panel panel-default">
      <div class="panel-heading">
		  ...
		  <div class="form-group">
		    <label for="exampleInputName2">兴趣类型管理&nbsp;&nbsp;&nbsp;兴趣类型</label>
		    <input type="text" class="form-control" value="${itype }" id="itype" placeholder="Jane Doe">
		  </div>
      ...
      <div class="panel-body">
        <table class="table table-bordered table-hover">
          ...
          <c:forEach items="${list}" var="Inte" varStatus="start">
          	  <tr>
          	  	<td><input type="checkbox" value="${Inte.inid}" /></td>
	            <td>${start.index + 1}</td>
	            <td>${Inte.itype}</td>
	          </tr>
          </c:forEach>
    ...
    <form action="${basePath}Inte/update" id="updateForm" method="post">
    	<input type="hidden" id="_Inid" name="Inid" value=""/>
    </form>
    
    <form action="${basePath}Inte/serch" id="pageForm" method="post">
    	<input type="hidden" id="NewPage" name="NewPage" value="">
		 <input type="hidden" id="_itype" name="itype" value="">
    </form>
	...
	<script type="text/javascript">
		function page(pagenum){
	   	...
	   	$("#find").click(function(){
	   		var url = "${basePath}Inte/"+$("#itype").val();
	   		$("#findForm").attr("action",url);
	   		$("#findForm").submit();
	   	});
	   	
	   	$("#add").click(function(){
	   		window.location.href="${basePath}Inte/add";
	   	});
	   	
	   	//$('input[type=checkbox]:checked')
	   	$("#delete").click(function(){
	   		if(check()){
	   			var url = "${basePath}Inte/"+$('input[type=checkbox]:checked').val();
	   	...
	   	$("#update").click(function(){
	   		if(check()){
	   			$("#_Inid").val($('input[type=checkbox]:checked').val());
	   			$("#updateForm").submit();
	   		}
	   	});
